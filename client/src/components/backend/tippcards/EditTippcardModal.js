...
class EditTippcardModal extends Component {
    ...
    openModal() {
        ...
        setTimeout(()=>{
            ...
            document.getElementById(this.props.tippcard.kind).checked = true;
            newTippcardKind=this.props.tippcard.kind;
    ...
    toggleKindCheckboxValue(name){
        ...
        this.validateForm();
        this.setState({isKindChanged: true});
    }
    ...
    handleSubmit (e) {
        ...
        firebase.database().ref('/tippcards/'+this.props.tippcard.id+'/').update(newTippcard).then(this.closeModal).then(
            setTimeout(()=>{
                this.props.getTippcards();
            },1000)
        );
    ...
    render() {
       ...
       for(let i=0;i<Object.keys(jsonOfChoosenCategories).length;i++) {
        ...
        let kindWarning = '';
        if(this.state.isKindChanged) {
            kindWarning =
                <div className="warning">Du hast die Art der Tippkarte geändert. Das ist unter Umständen nicht sinnvoll.
                    Sie war ursprünglich eine <strong>{this.props.tippcard.kind} - Karte</strong>.</div>;
        }
...
//})(
connect(mapStateToProps, {getTippcards, getInteractions, getCategories, getKinds })(EditTippcardModal)
//);
